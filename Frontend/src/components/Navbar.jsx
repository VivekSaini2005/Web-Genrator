      ...
      {showOptions && (
        <div className="order-3 w-full md:order-none md:w-auto md:absolute md:left-1/2 md:-translate-x-1/2 flex items-center justify-center">
          <div className="w-full md:w-auto overflow-x-auto overscroll-x-contain [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            <div className="inline-flex min-w-max items-center rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-0.5 sm:p-1 text-[var(--text-secondary)] shadow-sm transition-all duration-300">
              <button
                onClick={() => setActiveView("code")}
                className={`whitespace-nowrap px-2.5 py-1.5 sm:px-3 sm:py-1.5 md:px-4 md:py-2 text-[11px] sm:text-[12px] md:text-[13px] font-medium rounded-lg border-b-2 transition-all duration-300 transform hover:scale-[1.03] ${activeView === 'code' ? 'border-[var(--accent)] bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-sm' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)]'}`}
              >
                Code
              </button>
              <button
                onClick={() => setActiveView("preview")}
                className={`whitespace-nowrap px-2.5 py-1.5 sm:px-3 sm:py-1.5 md:px-4 md:py-2 text-[11px] sm:text-[12px] md:text-[13px] font-medium rounded-lg border-b-2 transition-all duration-300 transform hover:scale-[1.03] ${activeView === 'preview' ? 'border-[var(--accent)] bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-sm' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)]'}`}
              >
                Preview
              </button>
              <button
                onClick={() => setActiveView("chat")}
                className={`whitespace-nowrap px-2.5 py-1.5 sm:px-3 sm:py-1.5 md:px-4 md:py-2 text-[11px] sm:text-[12px] md:text-[13px] font-medium rounded-lg border-b-2 transition-all duration-300 transform hover:scale-[1.03] ${activeView === 'chat' ? 'border-[var(--accent)] bg-[var(--bg-primary)] text-[var(--text-primary)] shadow-sm' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)]'}`}
              >
                Chat
              </button>
            </div>
